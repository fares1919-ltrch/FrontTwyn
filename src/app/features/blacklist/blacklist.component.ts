...
export class BlacklistComponent implements OnInit {
    ...
    constructor(
        ...
        this.filterForm = this.fb.group({
            status: [''], // Empty string represents "All" in the filter logic
            processId: [''],
            fileName: ['']
        });
    }
    ...
    ngOnInit(): void {
        // Set the status filter to empty string (All) by default
    ...
    fetchAllDuplicates(): void {
        ...
        this.duplicateRecordService.getAllDuplicateRecords().subscribe({
            next: (records: DuplicateRecord[]) => {
                ...
                // Make sure we're showing all records by default
                this.filterForm.get('status')?.setValue('');
                this.applyFilters(); // Apply filters with "All" selected
    ...
    applyFilters(): void {
        ...
        this.filteredRecords = this.duplicateRecords.filter(record => {
            ...
            if (status && status !== '' && record.status !== status) {
                return false;
            }
     ...
     * @param record The duplicate record to delete
     */
    deleteDuplicateRecord(record: DuplicateRecord): void {
        // Show confirmation dialog first
        const dialogRef = this.dialog.open(ConfirmDialogComponent, {
            width: '400px',
            data: {
                title: 'Confirm Deletion',
                message: `Are you sure you want to delete this duplicate record for ${record.originalFileName}?`,
                confirmText: 'Delete',
                cancelText: 'Cancel'
            }
        });
        ...
        dialogRef.afterClosed().subscribe(result => {
            if (result) {
                this.duplicateRecordService.deleteDuplicateRecord(record.id).subscribe({
                    next: () => {
                        this.showNotification('Duplicate record deleted successfully', 'success');
                        // Refresh the list
                        if (this.processId) {
                            this.fetchDuplicatesByProcess(this.processId);
                        } else {
                    ...
                    error: (error) => {
                        ...
                        this.showNotification('Failed to delete duplicate record. Please try again.', 'error');
                    }
                });
            }
        });
