...
interface ExtendedFileDetails extends FileDetails {
    base64Preview?: string;
    previewStatus?: 'pending' | 'success' | 'error';
    previewError?: string;
}
...
export class deduplicationComponent implements OnInit {
        ...
        this.deduplicationService.getProcessDetails(formattedProcessId).subscribe({
            next: (process: Process) => {
                ...
                this.stats = [{
                    ...
                    subtext: `Files Processed: ${process.processedFiles} of ${process.totalFiles}`,
    ...
    loadImagePreviews(files: FileDetails[]): void {
        ...
        // Map FileDetails to ExtractedFile format
        const extractedFiles: ExtractedFile[] = files.map(file => ({
            id: file.id,
            fileName: file.fileName,
            filePath: file.filePath,
            status: file.status,
            base64Preview: (file as ExtendedFileDetails).base64Preview,
            base64String: file.base64String || '',
            previewStatus: (file as ExtendedFileDetails).previewStatus || 'pending',
            previewError: (file as ExtendedFileDetails).previewError
        }));
                ...
                if (processes && processes.length > 0) {
                    this.stats = processes.map((p) => {
                        ...
                        return {
                            ...
                            title: `Process ID: ${actualProcessId}`,
                            subtext: `Files Processed: ${p.processedFiles} of ${p.totalFiles}`,
                            processId: actualProcessId
                        };
                    });
     ...
     */
    pauseDeduplication(processId: string): void {
        // Check if the process is in a state that can be paused
        if (this.highlightedProcess && this.highlightedProcess.status?.toLowerCase() === 'paused') {
            const errorMsg = `Process is already paused.`;
            this.showNotification(errorMsg, 'info');
            return;
        }

        // Check if the process is not in 'In Processing' state
        if (this.highlightedProcess && this.highlightedProcess.status?.toLowerCase() !== 'in processing') {
            const errorMsg = `Cannot pause process because it is in ${this.highlightedProcess.status} state. Only processes in 'In Processing' state can be paused.`;
        ...
        this.showNotification('Pausing deduplication process...', 'info');

        this.deduplicationService.pauseDeduplication(processId).subscribe({
            next: (response: any) => {
                console.log('Deduplication paused:', response);
                this.isProcessing = false;
                this.errorMessage = null;
                this.hasError = false;

                // Format the response to match DeduplicationResponse interface
                this.deduplicationResult = {
                    success: true,
                    message: response.message || 'Process paused successfully',
                    processId: response.processId
                };

                this.showNotification(this.deduplicationResult.message, 'success');
            ...
            error: (error: any) => {
                ...
                // Refresh the process data to ensure UI is in sync with backend state
                if (this.highlightedProcessId) {
                    this.fetchProcessById(this.highlightedProcessId);
                }
            }
     ...
     */
    resumeDeduplication(processId: string): void {
        // Check if the process is already running
        if (this.highlightedProcess && this.highlightedProcess.status?.toLowerCase() === 'in processing') {
            const errorMsg = `Process is already running.`;
            this.showNotification(errorMsg, 'info');
            return;
        }

        // Check if the process is not in 'Paused' state
        if (this.highlightedProcess && this.highlightedProcess.status?.toLowerCase() !== 'paused') {
            const errorMsg = `Cannot resume process because it is in ${this.highlightedProcess.status} state. Only processes in 'Paused' state can be resumed.`;
            this.showNotification(errorMsg, 'error');
            this.deduplicationResult = {
                success: false,
                message: errorMsg
        ...
        this.showNotification('Resuming deduplication process...', 'info');

        this.deduplicationService.resumeDeduplication(processId).subscribe({
            next: (response: any) => {
                console.log('Deduplication resumed:', response);
                this.isProcessing = false;
                this.errorMessage = null;
                this.hasError = false;
                ...
                this.deduplicationResult = {
                    success: true,
                    message: response.message || 'Process resumed successfully',
                    processId: response.processId
                };
    ...
    /**
     * Navigate to the audit trail page for the current process
     */
    viewAuditTrail(): void {
        if (this.highlightedProcessId) {
            this.router.navigate(['/features/audit'], {
                queryParams: { processId: this.highlightedProcessId }
            });
        } else {
            this.showNotification('No process selected', 'error');
