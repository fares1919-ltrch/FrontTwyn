...
/**
 * Upload History component for displaying user's uploaded files and processes
 */
@Component({
...
export class UploadHistoryComponent implements OnInit, AfterViewInit {
  ...
  /**
   * Load user's upload history from the backend
   */
  loadProcesses(): void {
    this.isLoading = true;
    this.error = null;
    ...
    if (!currentUser) {
      this.error = 'You must be logged in to view upload history.';
