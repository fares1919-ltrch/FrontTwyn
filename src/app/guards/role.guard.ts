...
export class RoleGuard implements CanActivate {
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): boolean {
    ...
    if (!currentUser.isValidated) {
      // Only allow access to profile for unvalidated users
      if (state.url.includes('/features/profile')) {
        return true;
      }

      // For other routes, redirect unvalidated users to profile
    ...
    if (currentUser.isValidated) {
      // For admin features, only allow Admin and SuperAdmin
      ...
      // For SuperAdmin features, only allow SuperAdmin
