...
export class DuplicateRecordService {
  ...
  // Fix the deletePhoto method to use the correct URL format without DuplicatedRecords/ prefix
  deletePhoto(recordId: string, photoId: number): Observable<DuplicateRecord> {
    // Remove DuplicatedRecords/ prefix if present
    const cleanId = recordId.replace('DuplicatedRecords/', '');
    
    return this.http.delete<DuplicateRecord>(`${this.apiUrl}/DuplicateRecord/${cleanId}/photo/${photoId}`)
      .pipe(
        catchError(error => {
          console.error('Error deleting photo:', error);
          return throwError(() => new Error('Failed to delete photo. Please try again.'));
        })
      );
  }

  // Add method to delete entire duplicate record
  deleteDuplicateRecord(recordId: string): Observable<void> {
    // Remove DuplicatedRecords/ prefix if present
    const cleanId = recordId.replace('DuplicatedRecords/', '');
    
    return this.http.delete<void>(`${this.apiUrl}/DuplicateRecord/${cleanId}`)
      .pipe(
        catchError(error => {
          console.error('Error deleting duplicate record:', error);
          return throwError(() => new Error('Failed to delete duplicate record. Please try again.'));
        })
      );
  }
}
