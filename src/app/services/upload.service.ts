...
export class UploadService {
  ...
  getUploadException(exceptionId: string): Observable<any> {
    // Since there's no direct endpoint to get a single exception by ID,
    // we'll get all exceptions and filter for the one we want
    return this.exceptionService.getAllExceptions().pipe(
      map(exceptions => {
        // Find the exception with the matching ID
        const exception = exceptions.find(e => e.id === exceptionId ||
                                             (e.shortId && e.shortId === exceptionId));
  ...
  /**
   * Compose multiple image files into a tar.gz file
   * @param files Array of image files to compose
   * @returns Promise with the composed tar.gz file
   */
  async composeImagesIntoTarGz(files: File[]): Promise<File> {
    ...
    try {
      // Send files to backend for composition
      const response = await this.http.post<Blob>(
        `${this.apiUrl}/Uploading/compose-tar-gz`,
        formData,
        {
      ...
      if (!response || !response.body) {
        throw new Error('Failed to compose tar.gz file');
      }

      // Create a new File object from the response blob
      return new File(
        [response.body],
        'composed_images.tar.gz',
        { type: 'application/x-gzip' }
      );
    } catch (error: any) {
      console.error('Error composing tar.gz:', error);
      ...
      if (error.error instanceof Blob) {
        try {
          ...
          try {
            ...
            // If it's not valid JSON, just use the error text
            if (errorText && errorText.length > 0) {
              throw new Error(errorText);
            }
          }
        } catch (blobError) {
          // If we can't read the blob, fall back to the original error
      ...
      if (error.status === 400) {
        throw new Error('Invalid file format or unsupported image type. Please ensure all files are supported image formats (JPEG, PNG, GIF).');
      } else {
        throw new Error('Failed to compose tar.gz file: ' + (error.message || 'Unknown error'));
      }
    }
  }
}
